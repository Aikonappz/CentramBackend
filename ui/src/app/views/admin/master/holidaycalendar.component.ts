...
import { HolidayCalendarDataSource } from '../../../service/datasource/HolidayCalendarDataSource';
...
export class HolidayCalendarComponent implements OnInit {
  ...
  displayedColumns = ['account', 'location', 'year', 'action'];
  private datasource: HolidayCalendarDataSource;
  ...
  ngOnInit(): void {
    this.datasource = new HolidayCalendarDataSource(this.service);
  ...
  edit(loc: LocationVO) {
    this.router.navigate(['/admin/master/calendar/edit/' + loc.id]);
  }
  add() {
    this.router.navigate(['/admin/master/calendar/add']);
  }

  updateStatus(hc: HolidayCalendar) {
  ...
  download(obj: HolidayCalendar) {
    this.service
      .downloadholidayCalendarService(obj.id, {})
      .subscribe((data: any) => {
        //console.log(data);
