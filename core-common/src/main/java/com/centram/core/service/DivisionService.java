package com.centram.core.service;

import com.centram.common.dto.CommonProjection;
import com.centram.common.dto.LoggedInUser;
import com.centram.common.utility.PaginatedList;
import com.centram.core.repository.DivisionRepository;
import com.centram.core.repository.PositionRepository;
import com.centram.domain.Division;
import com.centram.domain.Division;
import com.centram.domain.enumarator.Status;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import javax.persistence.EntityNotFoundException;
import java.math.BigInteger;

@Service
public class DivisionService {

    @Autowired
    private DivisionRepository divisionRepository;

    @Transactional
    public Division save(Division Division) {
        LoggedInUser loggedInUser = (LoggedInUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return divisionRepository.save(Division);
    }

    @Transactional
    public Division getById(BigInteger id) {
        return divisionRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Division not found with id: " + id));
    }

    @Transactional
    public PaginatedList<CommonProjection> getAll(String name, Status status, Pageable pageable) {
        return new PaginatedList<CommonProjection>(divisionRepository.findAllBy(pageable));
    }
}
