...
public class MiscService {
        ...
        for (Map<String, String> data : dataList) {
            ...
            try {
                if (data.get("PRODUCT_CATEGORY") == null || data.get("PRODUCT_CATEGORY").trim().equals("")) {
                    ...
                    module = moduleService.getModuleByCustomerModuleName(LicenseType.ASSET, data.get("PRODUCT_CATEGORY").trim());
                    if (module == null) {
    ...
    @Transactional
    @Async("asyncExecutor")
    public void notifyIncidentCreationViaEmail(IncidentEmailVO incidentEmailVO) {
        /*need data for email*/
        List<com.centram.domain.Module> modules = new ArrayList<com.centram.domain.Module>();
        modules.add(incidentEmailVO.getModuleCategory());
        modules.add(incidentEmailVO.getModuleSubCategory());
    ...
    public void organisationUpdate(Organisation organisation, Boolean newEntity) {
        ...
        mailValues.put("recipients", Collections.singletonList(loggedInUser.getEmail()));
    ...
    public void assetWarrantyExpiration(Asset asset) {
        List<UserVO> userVOS = userService.getUsersByRolesAndOrganisation(List.of("ORG_ASSET_ADMIN"), asset.getOrganisation().getId());
        Map<String, Object> mailValues = new HashMap<String, Object>();
    ...
    public void assetValidityExpiration(Incident incident) {
        ...
        List<UserVO> userVOS = List.of(userVO);
        Map<String, Object> mailValues = new HashMap<String, Object>();
    ...
    public void sendOnboardMail(UserVO userVO, Map<String, Object> mailValues) {
        if (userVO.getRoles().contains(BigInteger.valueOf(Long.valueOf("3")))) {
            ...
            mailValues.put("valid_to", organisation.getLicenseEnd().format(DateTimeFormatter.ofPattern(dateFormat)));
            mailValues.put("recipientType", "Organization Admin");
            mailValues.put("recipients", Collections.singletonList(userVO.getEmail()));
            mailValues.put("userToNotify", Collections.singletonList(userVO));
    ...
    public void notifyParticipant(final ProjectUatScriptDetail projectUatScriptDetail) throws JsonProcessingException, InterruptedException {
        ...
        User user;
        Boolean hasMultiCustomer = project.getStakeHolders().size() > 1;
        String customerDetail = projectUatScriptDetail.getCustomerUser() != null ? projectUatScriptDetail.getCustomerUser().getFirstName() + " " + projectUatScriptDetail.getCustomerUser().getLastName() + "/" + projectUatScriptDetail.getCustomerUser().getEmail() : null;
        String uatScriptAppender = hasMultiCustomer ? "/" + customerDetail : "";
        Map<String, Object> emailValues = new LinkedHashMap<String, Object>();
        emailValues.put("uatScript", projectUatScript.getTestScriptName() + " [" + projectUatScript.getTestCaseId() + "]" + uatScriptAppender);
        ...
        for (String customerEmail : project.getStakeHolders()) {
            if (projectUatScriptDetail.getCustomerUser() != null && projectUatScriptDetail.getCustomerUser().getEmail().equals(customerEmail)) {
    ...
    @Async("asyncExecutor")
    @Transactional(propagation = Propagation.REQUIRED, readOnly = true)
    public void notifyUatScriptUpload(ProjectUat projectUat) throws JsonProcessingException, InterruptedException {
        ProjectUATVO projectUATVO = new ProjectUATVO();
        projectUATVO.setNotifications(new ArrayList<Notification>());
        List<String> usersEmail = new ArrayList<String>();
        User user;
    ...
    @Async("asyncExecutor")
    @Transactional(propagation = Propagation.REQUIRED, readOnly = true)
    public void notifyUatScriptCompletion(final ProjectUatScript projectUatScript) throws JsonProcessingException, InterruptedException {
        ProjectUATVO projectUATVO = new ProjectUATVO();
        projectUATVO.setNotifications(new ArrayList<Notification>());
        List<String> usersEmail = new ArrayList<String>();
        User user;
        ...
        Boolean hasMultiCustomer = project.getStakeHolders().size() > 1;
        String customerDetail = projectUatScript.getCustomerUser() != null ? projectUatScript.getCustomerUser().getFirstName() + " " + projectUatScript.getCustomerUser().getLastName() + "/" + projectUatScript.getCustomerUser().getEmail() : null;
        String uatScriptAppender = hasMultiCustomer ? "/" + customerDetail : "";
