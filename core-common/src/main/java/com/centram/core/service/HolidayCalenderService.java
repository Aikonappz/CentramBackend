...
public class HolidayCalenderService {
    ...
    @Transactional(readOnly = true)
    public List<Holiday> getHolidaysByYearAndLocation(BigInteger accountId, BigInteger location, String year) {
        LoggedInUser loggedInUser = (LoggedInUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        HolidayCalender holidayCalender = holidayCalenderRepository.getHolidayCalenderByYear(
                accountId, year, location, loggedInUser.getOrganisationId()
        );
